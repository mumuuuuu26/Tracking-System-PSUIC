    ...
    return (
        <ITWrapper>
            <div className="flex flex-col h-[calc(100vh-6rem)]">
                ...
                {/* Filter & Search Bar */}
                <div className="bg-white dark:bg-[#1a2f4e] p-4 rounded-2xl shadow-sm dark:shadow-lg border border-gray-100 dark:border-blue-800/30 mb-6 flex flex-col md:flex-row gap-4 items-center justify-between sticky top-0 z-10">
                        ...
                        {["All", "Not Start", "In Progress", "Completed", "Rejected"].map((status) => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all border ${activeFilter === status
                                    ? "bg-[#1e2e4a] dark:bg-blue-600 text-white border-[#1e2e4a] dark:border-blue-600 shadow-md shadow-blue-900/20 dark:shadow-blue-900/30"
                                    : "bg-gray-50 dark:bg-blue-900/20 text-gray-500 dark:text-blue-300/70 border-gray-200 dark:border-blue-800/40 hover:bg-gray-100 dark:hover:bg-blue-900/30"
                                    }`}
                            >
                                {status}
                            </button>
                    ...
                    <div className="relative w-full md:w-64">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-blue-300/50" size={18} />
                        <input
                            type="text"
                            placeholder="Search tickets..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full pl-10 pr-4 py-2.5 bg-gray-50 dark:bg-[#0d1b2a] border border-gray-200 dark:border-blue-800/40 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/30 transition-all font-medium text-gray-800 dark:text-white placeholder-gray-400 dark:placeholder-blue-400/40"
                        />
                    </div>
                </div>
...
const TicketCard = ({ ticket, navigate }) => {
    const getStatusConfig = (status) => {
        switch (status) {
            case "not_start":
            case "pending":
                return { label: "Not Started", className: "border-gray-400 dark:border-gray-600/50 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/30" };
            case "in_progress":
                return { label: "In Progress", className: "border-blue-500 dark:border-blue-600/50 text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20" };
            case "completed":
                return { label: "Completed", className: "border-green-500 dark:border-green-600/50 text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20" };
            case "rejected":
                return { label: "Rejected", className: "border-red-400 dark:border-red-600/50 text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-900/20" };
            default:
                return { label: "Not Started", className: "border-gray-400 dark:border-gray-600/50 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/30" };
        }
    };
    ...
    return (
        <div
            ...
            className="bg-white dark:bg-[#1a2f4e] rounded-3xl p-6 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:shadow-lg border border-gray-100 dark:border-blue-800/30 relative overflow-hidden transition-all duration-300 hover:shadow-lg dark:hover:shadow-blue-900/20 cursor-pointer"
        >
            {/* Header: Category & Status */}
            <div className="flex justify-between items-start mb-3">
                <h3 className="text-xl font-bold text-[#193C6C] dark:text-white tracking-tight">
            ...
            {/* Body: Title & Location */}
            <div className="mb-6">
                <h4 className="text-lg font-bold text-slate-800 dark:text-gray-200 mb-2 line-clamp-2 leading-relaxed">
                    {ticket.title || ticket.description || "-"}
                </h4>
                <p className="text-gray-400 dark:text-blue-300/50 text-xs font-semibold bg-gray-50 dark:bg-blue-900/20 inline-block px-2 py-1 rounded-md">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm font-bold text-gray-500 dark:text-blue-300/60">
                    <span>
                ...
                <span className="text-sm font-bold text-[#193C6C] dark:text-white">
