...
const UserManagement = () => {
    ...
    const loadUsers = async () => {
        try {
            // Mapping filter to backend role query
            // Mockup tabs: All, Students, Lecturers, Staff
            // Our roles: user, it_support, admin
            // This is a simplification; "Students" and "Lecturers" are not distinct roles in our schema yet,
            // but we can filter by role or assume 'user' covers both for now.
            // If we want exact match, we might need a type field.
            // For now, I'll map: "Students" -> role=user, "Staff" -> role=it_support/admin?
            // Let's stick to simple Role filtering for now or use the 'role' param if it matches.
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2">
                        {["All", "Students", "Staff"].map((tab) => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${filter === tab
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className={`px-2 py-0.5 rounded text-xs font-semibold ${user.enabled ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className={`text-sm font-medium px-3 py-1.5 rounded-lg border transition-colors ${user.enabled ? 'text-red-600 border-red-200 hover:bg-red-50' : 'text-green-600 border-green-200 hover:bg-green-50'}`}
                                >
